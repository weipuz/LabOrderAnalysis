...
import org.apache.spark.mllib.fpm.AssociationRulesWithLift.Rule
 ...
 */
@Experimental
class AssociationRulesWithLift private[fpm] (
  ...
  def run[Item: ClassTag](freqItemsets: RDD[FreqItemset[Item]]): RDD[Rule[Item]] = {
    ...
    candidates.join(freqItemsets.map(x => (x.items.toSeq, x.freq)))
     ...
     (consequent, (antecendent, freqUnion, freqAntecedent))
      ...
      new Rule(antecendent.toArray, consequent.toArray, freqUnion, freqAntecedent, freqConsequent)
...
object AssociationRulesWithLift {
  ...
  class Rule[Item] private[fpm] (
      ...
      freqConsequent: Double) extends Serializable {

    def confidence: Double = freqUnion.toDouble / freqAntecedent
    def lift: Double = freqUnion.toDouble / (freqAntecedent * freqConsequent)
